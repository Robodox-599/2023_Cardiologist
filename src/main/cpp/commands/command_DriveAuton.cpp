...
command_DriveAuton::command_DriveAuton(subsystem_DriveTrain* DriveTrain, subsystem_PoseTracker* PoseTracker, std::string m_TrajFilePath, bool ToReset):
m_DriveTrain{DriveTrain}, m_PoseTracker{PoseTracker}, m_ToReset{ToReset},  m_DriveController{AutoConstants::XPID, AutoConstants::YPID, AutoConstants::ThetaPID} {
  ...
  if(m_TrajFilePath.starts_with("Yes^3") ){
    m_Trajectory = pathplanner::PathPlanner::loadPath(m_TrajFilePath, pathplanner::PathConstraints(4_mps, units::meters_per_second_squared_t{4}) );  

  }else{
    m_Trajectory = pathplanner::PathPlanner::loadPath(m_TrajFilePath, pathplanner::PathConstraints(AutoConstants::MaxSpeed, AutoConstants::MaxAccel) );  
