...
command_DriveAuton::command_DriveAuton(subsystem_DriveTrain* DriveTrain, subsystem_PoseTracker* PoseTracker, std::string TrajFilePath, bool ToReset, bool IsCustomMotion):
m_DriveTrain{DriveTrain}, m_PoseTracker{PoseTracker}, m_ToReset{ToReset}, m_IsCustomMotion{IsCustomMotion}, m_DriveController{AutoConstants::XPID, AutoConstants::YPID, AutoConstants::ThetaPID} {
  ...
  if( !IsCustomMotion ){
    ...
    m_Trajectory = pathplanner::PathPlanner::loadPath( TrajFilePath, pathplanner::PathConstraints(0.1_mps, units::meters_per_second_squared_t{0.1}) );
  }
