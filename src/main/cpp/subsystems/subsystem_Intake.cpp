...
{
    units::meter_t DistanceFromTarget = YBotPosition - IntakeConstants::CenterToStowedIntake - IntakeConstants::DistanceToHighCubeY;
    // x = t(v_0)cos(theta)
    // y = -9.81(t)^2 + t(v_0)sin(theta) + h
    // t(v_0)sin(theta) = y + 9.81(t)^2 - h
    
    double HighCubeShootVelocity = (IntakeConstants::DistanceToHighCubeZ.value() - IntakeConstants::InitialHeight.value() + 9.81 * pow(IntakeConstants::CubeFlightTime.value(), 2))/(IntakeConstants::CubeFlightTime.value() * sin(IntakeConstants::AngleOfLaunch.value()));
    m_IntakeMotorPID.SetReference(HighCubeShootVelocity, rev::ControlType::kVelocity);
}

void subsystem_Intake::SetMidCubeDynamicVelocity(units::meter_t YBotPosition)
{
    units::meter_t DistanceFromTarget = YBotPosition - IntakeConstants::CenterToStowedIntake - IntakeConstants::DistanceToMidCubeY;
    double MidCubeShootVelocity = (IntakeConstants::DistanceToMidCubeZ.value() - IntakeConstants::InitialHeight.value() + 9.81 * pow(IntakeConstants::CubeFlightTime.value(), 2))/(IntakeConstants::CubeFlightTime.value() * sin(IntakeConstants::AngleOfLaunch.value()));
    m_IntakeMotorPID.SetReference(MidCubeShootVelocity, rev::ControlType::kVelocity);
